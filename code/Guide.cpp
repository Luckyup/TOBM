...
// typedef unordered_map<pii,int> HashMap;
...
//typedef map<pii, int, hash_compare<pii, piicomp>> HashMap;
...
void init() {
	...
	for (int i=0; i<workerN; ++i) {
		sid = spatioId(workers[i].loc, gridSize, gridPer);	//网格号
		tid = temporalId(workers[i].at, slotSize);	//时间戳
		pii g = make_pair(sid, tid);
		wHash[g]++;	//增加worker结点，结点数自加一
	}
	...
	for (int i=0; i<taskN; ++i) {
		...
		pii g = make_pair(sid, tid);
		tHash[g]++;		//task该时空戳结点自加1
	}
	
	init_network();		//初始化网络部分
}
...
void addEdge(int u, int v, int c) {		
	E.push_back(edge_t(v, c, head[u]));		//表示u到v的流量为c，以及加上上一个定义的该边结点
	head[u] = E.size() - 1;		

	E.push_back(edge_t(u, 0, head[v]));		//表示v到u的流量为0
	head[v] = E.size() - 1;				
}

void init_network() {
	HashMap::iterator wIter, tIter;		//增加迭代器
	int k;
	
	wnodeN = wHash.size();	//prediction结点的数量“一个网格中只有一个task结点和一个worker结点”
	tnodeN = tHash.size();
	st = wnodeN + tnodeN;		// 哦，这是start结点
	ed = wnodeN + tnodeN + 1;		//这就是 sink结点，那罗或多
	int vertexN = wnodeN + tnodeN + 2;		//图中结点（包括sink和start）数

	dis.clear();	//vector int
	dis.resize(vertexN, 0);	// resize int vertexNum
	head.clear();		// vector int
	head.resize(vertexN, -1);  // 头结点初始设置为-1，标识无头结点
	head_.clear();				
	head_.resize(vertexN, -1);	//同样设置成-1，标识没有后继
	E.clear();		// edge有 v f nxt三个参数
	
	for (k=0,wIter=wHash.begin(); wIter!=wHash.end(); ++k,++wIter) {		// k是 worker结点的下标
		addEdge(st, k, wIter->second);		// wIter->second是worker结点的worker数量
	}
	for (k=0,tIter=tHash.begin(); tIter!=tHash.end(); ++k,++tIter) {
		addEdge(wnodeN+k, ed, tIter->second);	//从task到sink，end结点，同样的使用task数量小者作为流量
	}
	
	int i, j;
	for (i=0,wIter=wHash.begin(); wIter!=wHash.end(); ++i,++wIter) {
		for (j=0,tIter=tHash.begin(); tIter!=tHash.end(); ++j,++tIter) {
			if (judge(wIter->first, tIter->first)) {		//首先需要确定两者是否可以到达
				addEdge(i, wnodeN+j, min(wIter->second, tIter->second));	//选取两者数量小者作为流量
			}
		}
	}
}
...
void dumpGuide(int maxFlow) {
	...
	for (int i=0; i<wnodeN; ++i) {
		M.push_back(vector<int>());		//对每个结点建立一个vector用来存取目标task
	}
	for (HashMap::iterator iter=wHash.begin(); iter!=wHash.end(); ++iter) {
		wps.push_back(iter->first);		//将worker的grid和时间戳进行保存
		wcaps.push_back(iter->second);	//将worker的数量进行保存
	}
	...
	for (u=0; u<wnodeN; ++u) {
		...
		for (k=0; k < M[u].size(); ++k) {
			...
			// 第1行，输出一个总数 
			// 第2到n行 首先是 worker的空间号以及时间号，之后是task的空间号和时间号 
		}
	}
